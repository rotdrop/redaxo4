...
if ($FUNC_UPDATE != '' || $FUNC_APPLY != '') {
    ...
    if ($userpsw != '') {
        ...
        if ($userpsw == '') {
            $warnings[] = $I18N->msg('user_psw_missing');
    ...
    if ($adduser->getRows() == 0 and $userlogin != '' and $userpsw != '') {
        $adduser = rex_sql::factory();
        ...
        if ($userlogin == '') {
            $warnings[] = $I18N->msg('user_login_missing');

        }

        if ($adduser->getRows() > 0) {
...
if (count($warnings) > 0) {
    echo rex_warning(implode('<br />', $warnings));
}
